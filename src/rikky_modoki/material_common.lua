return function(draw)
  ...
  local function gpu_material(state, count, args)
    ...
    obj.pixelshader(
      "rikky_material_base" .. shader_script,
      "object",
      { state.original },
      gpu_constants(geometry, state.base),
      "copy"
    )
    if #state.lights == 0 then
      return true
    end
    local partition = 0
    if state.hq and count <= 8 then
    ...
    for _, light in ipairs(state.lights) do
      local constants = {}
      for _, value in ipairs(light.constants) do
        constants[#constants + 1] = value
      end
      for _, value in ipairs({ state.damping, partition, state.width, state.height }) do
        constants[#constants + 1] = value
      end
      -- The untextured shader does not sample t0; bind the owned original explicitly.
      local texture = state.original
      if light.texture ~= nil then
        texture = light.texture
      end
      obj.computeshader(
        "rikky_material_light" .. shader_script,
        { diffuse, specular },
        { texture },
        gpu_constants(geometry, constants),
        math.ceil(width / 8),
        math.ceil(height / 8),
        1
      )
      obj.pixelshader(
        "rikky_material_apply" .. shader_script,
        "object",
        { state.original, "object", diffuse, specular },
        gpu_constants(geometry, { partition, 0, 0, 0 }),
        "copy"
      )
    end
    return true
