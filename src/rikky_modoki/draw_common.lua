return function(module)
  ...
  local function gpu_copy(destination, source)
    assert(
      obj.copybuffer(destination, source),
  ...
  local function gpu_constants(geometry, extra)
    local values = {}
    for _, value in ipairs(geometry) do
  ...
  local function glass_release_state(state)
    ...
    if state ~= nil then
      state.original, state.background = nil, nil
    end
  end
  ...
  local function draw_arguments(...)
    ...
    for i = 1, count do
      args[i] = finite_number(tonumber(args[i]))
    end
    return count, args
  end
  ...
  local function draw_processed_image(state, count, args, render)
    ...
    local ok, err = pcall(function()
      ...
      if state.blur > 0 then
        obj.effect("ぼかし", "範囲", state.blur, "サイズ固定", 1)
      end
      if count > 8 then
