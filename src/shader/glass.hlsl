...
float4 rikky_glass(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target {
    ...
    // Smooth radial barrel/pincushion distortion; identity when lens is disabled.
    float2 q = uv * 2 - 1;
    float strength = extra[1].w == 1 ? -0.25 : (extra[1].w == 2 ? 0.25 : 0);
    uv = (q * (1 + strength * dot(q, q)) + 1) * 0.5;
